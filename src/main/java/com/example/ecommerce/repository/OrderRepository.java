...
public class OrderRepository {
    ...
    public static void updateProductQuantity(int orderId, int prodId, int quantity) throws SQLException {
        try {
            ...
            // Create an UPDATE statement to update the product quantity
            ...
            if (rowsAffected > 0) {
                ...
                String subtotalUpdateSql = "UPDATE orderproducts SET total = (SELECT product_qty * price FROM orderproducts INNER JOIN products ON orderproducts.product_id = products.product_id WHERE orderproducts.order_id = ? AND orderproducts.product_id = ?) WHERE order_id = ? AND product_id = ?";
                preparedStatement = connection.prepareStatement(subtotalUpdateSql);
                preparedStatement.setInt(1, orderId);
                preparedStatement.setInt(2, prodId);
                preparedStatement.setInt(3, orderId);
                preparedStatement.setInt(4, prodId);

                int subtotalRowsAffected = preparedStatement.executeUpdate();
                if (subtotalRowsAffected > 0) {
                    System.out.println("Subtotal updated successfully!");
                } else {
                    System.out.println("No subtotal updated!");
                }
            } else {
